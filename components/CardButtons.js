...
const CardButtons = ({ state, counter, dispatch }) => {
  return (
    <>
      {countItem(state, counter.id) > 1 ? (
        <AiOutlineMinus
          className={styles.changeItemsValue}
          onClick={() => dispatch({ type: "DECREASE_ITEM", payload: counter })}
        />
      ) : (
        <FiTrash2
          className={styles.changeItemsValue}
          onClick={() =>
            dispatch({ type: "REMOVE_FROM_CART", payload: counter })
          }
        />
      )}
      ...
      <AiOutlinePlus
        className={styles.changeItemsValue}
        onClick={() => dispatch({ type: "INCREASE_ITEM", payload: counter })}
      />
    </>
  );
};
